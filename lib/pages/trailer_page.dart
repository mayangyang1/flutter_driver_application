...
class TrailerPage extends StatefulWidget {
  final truckCode;
  TrailerPage({key:Key, this.truckCode});
  @override
...
class _TrailerPageState extends State<TrailerPage> {
  ...
  _getTrailerPageMessage() {//获取挂车辆信息
    getAjax('trailerInfo', '?platformTruckCode=${widget.truckCode}', context).then((res){
      if(res == null){
        return ;
      }
      if(res['code'] == 200 && res['content'].length > 0) {
        mainTruckObj = res['content'];
  ...
  _searchTruckLiceseNo(truckLicenseNo) {//查询车辆
    String stringParams = '?truckLicenseNo=$truckLicenseNo&size=1&isTrailer=true';
  ...
  _rebindTruck(truckCode) {//绑定车辆
    postAjaxStr('$serviceUrl/driver-api/driver-api/truck/rebind/$truckCode', {'truckCode':truckCode, 'isTrailer': true}, context).then((res){
      if(res['code'] == 200) {
